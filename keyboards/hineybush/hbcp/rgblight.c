...
void sethsv_raw(uint8_t hue, uint8_t sat, uint8_t val, LED_TYPE *led1) {
    ...
    if (led1 == &led[RGBLIGHT_FLED1]) {
        ...
        fled_hs[1].hue = hue;
        fled_hs[1].sat = sat;
    } else if (led1 == &led[RGBLIGHT_FLED3]) {
        fled_hs[2].hue = hue;
        fled_hs[2].sat = sat;
    }
...
void rgblight_sethsv_noeeprom_old(uint8_t hue, uint8_t sat, uint8_t val) {
    if (rgblight_config.enable) {
        // HBCP custom code
        fled_hs[0].hue = fled_hs[1].hue = fled_hs[2].hue = hue;
        fled_hs[0].sat = fled_hs[1].sat = fled_hs[2].sat = sat;
...
#    endif
    ...
    switch (fled_mode) {
      ...
      case FLED_RGB:
        ...
        if (fled_hs[2].hue == 0 && fled_hs[2].hue == 0 &&
                (rgblight_status.base_mode == RGBLIGHT_MODE_SNAKE ||
                rgblight_status.base_mode == RGBLIGHT_MODE_KNIGHT))
            setrgb(0, 0, 0, &led[RGBLIGHT_FLED3]);
        else
            sethsv(fled_hs[2].hue, fled_hs[2].sat, fled_val, &led[RGBLIGHT_FLED3]);
      break;
...
#    endif
    ...
    // HBCP custom
    fled_hs[0].hue = fled_hs[1].hue = fled_hs[2].hue = 0;
    fled_hs[0].sat = fled_hs[1].sat = fled_hs[2].hue = 0;
...
#    endif
    ...
    for (i = 0; i < RGBLIGHT_EFFECT_KNIGHT_LED_NUM; i++) {
        ...
        if (i >= low_bound && i <= high_bound) {
            ...
            } else if (cur == RGBLIGHT_FLED3) {
                fled_hs[2].hue = fled_hs[2].sat = 0;
            }
